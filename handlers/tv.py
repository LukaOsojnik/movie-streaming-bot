...
from search import get_episode_numbers, get_episode_results, build_magnet, format_size
...
async def season_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not episodes:
        await query_cb.message.reply_text(f"❌ No episodes found for Season {season}")
